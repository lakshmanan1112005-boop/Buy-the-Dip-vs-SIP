...
class StrategyBacktester:
    ...
    def print_summary(self, strategy_name, cash, final_val, total_shares, max_dd, budget=None):
        ...
        print(f"Total Cash Deployed:   ${cash:,.2f}")
        print(f"Final Portfolio Value: ${final_val:,.2f}")
        print(f"Net Profit/Loss:       ${profit:,.2f}")
        print(f"Strategy Return:       {ret:.2f}%")
...
bt.print_summary("BUY THE DIP", bt.df['Btd_Total_Invested'].iloc[-1], bt.df['Btd_Portfolio_Value'].iloc[-1], bt.df['Btd_Total_Shares'].iloc[-1], -20.0) # Placeholder for actual DD calc
bt.print_summary("MONTHLY SIP ($500/mo)", bt.df['Sip_Total_Invested'].iloc[-1], bt.df['Sip_Portfolio_Value'].iloc[-1], bt.df['Sip_Total_Shares'].iloc[-1], -15.0)

# Plotting
plt.figure(figsize=(14, 7))
plt.plot(bt.df.index, bt.df['Close'], label='S&P 500', color='royalblue', alpha=0.4)
plt.scatter(bt.df[bt.df['Btd_Invested_Amt']>0].index, bt.df.loc[bt.df['Btd_Invested_Amt']>0, 'Close'], color='green', label='BTD Buy', marker='^')
plt.title('Strategy Evaluation Comparison')
plt.legend()
plt.show()
